...
for rep = 1:numRepition
    ...
    while (i<=N)
        ...
        % be loaded before.
        [T,timestamp] = GetLocatorTransformMatrix(jTcpObj, name);
        if (sum(T(:))~=1)
            absPtNum = i+(rep-1)*N;
            TAll(:,absPtNum) = T(:);
            ...
            hp3 = plot3(p(1,absPtNum),p(2,absPtNum),p(3,absPtNum),'ko');
            title(['rep: ' num2str(rep) ', measurements so far ' num2str(absPtNum)]);
            
            prompt = 'Do you want to save the measured data for later use? Y/N [Y]: ';
            str = input(prompt,'s');
            if strcmpi(str,'n')
                delete(hp3);
                pause;
                disp('Press any key to continue to the take the same measurement...')
            else
                % wait 0.001 seconds
